...
const codeBeforeReturn = code.slice(0, returnStart);
...
// Extracted Component for Social Login Buttons
const SocialLoginButtons = ({ language, handleSocialSignIn, isSubmitting }: any) => (
  <div className="flex flex-col gap-2 mt-4 border-t border-zinc-800 pt-4">
    <div className="relative mb-2">
      ...
      <div className="relative flex justify-center text-[10px] font-mono">
        <span className="bg-zinc-900 px-2 text-zinc-500">{language === 'fr' ? 'OU CONTINUER AVEC' : 'OR CONTINUE WITH'}</span>
      </div>
    </div>
...
fs.writeFileSync('src/pages/AuthPage.tsx', codeBeforeReturn + newReturnBlock);
