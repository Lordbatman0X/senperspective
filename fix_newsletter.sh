sed -i '/<form onSubmit={handleSubmit}/i\
          {!isSuccess && (\
            <>\
              <button\
                type="button"\
                onClick={async () => {\
                  try {\
                    const { loginWithGoogle } = await import("../contexts/AuthContext").then(m => m.useAuth());\
                    await loginWithGoogle();\
                  } catch (e) { console.error(e); }\
                }}\
                className="w-full py-3 bg-white text-black text-xs font-bold uppercase tracking-wider rounded-xl hover:opacity-90 transition-opacity cursor-pointer flex items-center justify-center gap-2 mb-3"\
              >\
