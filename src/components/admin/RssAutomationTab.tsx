    ...
    <div className="space-y-6 text-zinc-100 font-sans" id="newsroom-hub-root">
      ...
      {activeNewsroomTab === 'scheduler' && (
        <div className="bg-gradient-to-br from-zinc-900 via-zinc-900 to-zinc-950 border border-orange-500/30 rounded-3xl p-6 shadow-2xl space-y-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-zinc-800 pb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-zinc-400 mt-0.5">
                  {isFr 
                    ? 'Incorporez automatiquement des dépêches en brouillons. Publication contrôlée exclusivement après validation dans le panneau.'
                    : 'Automatically ingest stories as drafts. Publishing is strictly controlled via manual admin approval.'}
                </p>
              </div>
            </div>
          ...
          {/* Strict Moderation Policy Notice */}
          <div className="bg-amber-950/30 border border-amber-500/30 rounded-2xl p-3.5 flex items-center gap-3 text-xs text-amber-200/90 font-sans">
            <ShieldCheck size={18} className="text-amber-400 shrink-0" />
            <span>
              {isFr 
                ? "🔒 Modération éditoriale stricte : Tout article généré par le planificateur est enregistré en statut brouillon (isPublished: false). Seuls les articles que vous validez ou publiez manuellement dans le panneau seront visibles par vos lecteurs."
                : "🔒 Strict editorial moderation: Every article generated by the planner is created as an unpublished draft (isPublished: false). Only articles you manually approve or publish in the admin panel will go live."}
            </span>
          </div>
